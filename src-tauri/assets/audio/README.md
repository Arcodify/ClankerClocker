...
(see `src-tauri/src/audio.rs`, `src-tauri/src/lib.rs` / `commands.rs`).
...
Files are compiled into the binary via `include_bytes!` and decoded with `rodio`/`symphonia`,
so only mp3 is supported. Rebuild after changing a file.
